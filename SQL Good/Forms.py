from wtforms import Form, StringField, BooleanField, IntegerField, SelectField, SelectMultipleField, validators, widgets
...
class CreateTutorAccount(Form):
    ...
    tutor_age = IntegerField('Age', validators=[Regexp(regex='/^(2[1-9]|[2-6]\d)$/gm', message="Age must be between 20 and 70 only (not inclusive)")])
    tutor_mobile = StringField('Mobile Number (+65)', validators=[Regexp(regex=r"\[|6|8|9|]\d{7}", message="Mobile Number can only begin with '6', '8' or '9'. A valid mobile number should contain 8 digits.")])
